...
const RoomSelectButton = ({ selectButton, selectButtonHidden, updateGrandTotal, showDiscountDiv, scrollWindow, checkAvailabilityData }) => {
    // console.log(selectButton, selectButtonHidden, updateGrandTotal, showDiscountDiv, scrollWindow)
    useEffect(() => {
    ...
    */
    const reRenderComponent = () => {
        let findSelectedButtons = document.getElementsByClassName('roomSelectButton');
        ...
        // addon remove all
        if (document.getElementsByClassName('removeAddon')) {
            let removeAddon = document.getElementsByClassName('removeAddon');
            for (let f = 0; f < removeAddon.length; f++) {
                // console.log(removeAddon[f].style.display);
                if ( removeAddon[f].value === 'Remove' && (removeAddon[f].style.display === 'block' || removeAddon[f].style.display === '')) {
                    document.getElementById(removeAddon[f].id).click();
                }
            }
        }

        // addon click on MINUS sign when the addon is of type PER ITEM
        if (document.getElementsByClassName('addonsminus')) {
            let removeMinusSign = document.getElementsByClassName('addonsminus');
            for (let r = 0; r < removeMinusSign.length; r++) {
                if (document.getElementById(removeMinusSign[r].id)) {
                    // console.log(document.getElementById(removeMinusSign[r].id));
                    document.getElementById(removeMinusSign[r].id).parentElement.click();
                }
            }
        }
    ...
    function roomSelection(i, idPostfix, valueRoomTypeId, nextId, isDealApplied, data) {
        // console.log('roomSelection: ', i, idPostfix, valueRoomTypeId, nextId, isDealApplied)
                ...
                for (let rL = 1; rL <= noOfRooms; rL++) {
                    if (rL != i && document.getElementById('isRoomSelected' + rL).value === '0') {
                        ...
                        if (content !== 'undefined') {
                            let contentHtml = `<div className="row m-xs-5">${content}</div>`
            ...
            if (showOnlyRatePlanForOtherRoom === '1') {
                ...
                for (let e = 2; e <= document.getElementById('noOfRooms').value; e++) {
                    ...
                    for (let p = 0; p < promoHide.length; p++) {
                        promoHide[p].style.display = 'block';
                    }
            ...
            if (roomLeft === 0) {
                ...
                for (let k = 1; k <= noOfRooms; k++) {
                    ...
                    if (k !== i && document.getElementById('isRoomSelected' + k).value === '0') {
                        ...
                        if (document.getElementById('buttonAvailable' + k + valueRoomTypeId)) {
    ...
    function popupOpen(id, roomNumber, noOfRooms) {
        console.log(id, roomNumber, noOfRooms);
        // alert(noOfRooms);
        let current = 0;
    ...
    function updatePriceTable(roomNumber, avgBaseRate, avgDiscountRate, extraAdultCharge, extraChildCharge, roomTypeName, roomTypeId, otherDiscountName, otherDiscountID) {
        ...
        if (otherDiscountName != 'undefined') {
            ...
            let hmtlToAdd = otherDiscountName + `<a href="javascript:void(0)" id=${'showPolicy_' + roomNumber}> Rate Policy</a>`;
            /*
            ...
            */
            ...
            document.getElementById('showPolicy_' + roomNumber).addEventListener('click', function () {
                popupOpen(roomNumber + roomTypeId, roomNumber, document.getElementById('noOfRooms').value);
            })
        } else {
            ...
            // console.log(specialCancellationPolicy)
            if (!isNaN(specialCancellationPolicy) && specialCancellationPolicy !== 0) {
                let hmtlToAdd = `<a href='javascript:void(0)' id=${'cancelPolicy' + roomNumber + roomTypeId} >Cancellation Policy</a>`;
                ...
                */
                ...
                document.getElementById('cancelPolicy' + roomNumber + roomTypeId).addEventListener('click', function () {
                    popupOpen(roomNumber + roomTypeId, roomNumber, document.getElementById('noOfRooms').value);
                })
            } else {
                let hmtlToAdd = `<a href='javascript:void(0)' id=${'standardPolicy_' + roomNumber}> Rate Policy</a>`;
                /*
                ...
                */
                ...
                document.getElementById('standardPolicy_' + roomNumber).addEventListener('click', function () {
                    popupOpen(roomNumber + roomTypeId, roomNumber, document.getElementById('noOfRooms').value);
                })
            }
        }
    ...
    if (selectButton && selectButton.length > 0) {
        var i = selectButton[0]
        var idPostfix = selectButton[1]
        var valueRoomTypeId = selectButton[2]
        var nextId = selectButton[3]
        var isDealApplied = selectButton[4]
    }
    if (selectButtonHidden && selectButtonHidden.length > 0) {
        var roomBaseCharge = selectButtonHidden[1]
        var roomDiscountCharge = selectButtonHidden[2]
        var extraAdultCharge = selectButtonHidden[3]
        var extraChildCharge = selectButtonHidden[4]
        var valueRoomTypeName = selectButtonHidden[5]
        if (selectButtonHidden[7] != '') {
            var BestDiscountName = selectButtonHidden[7]
        } else {
            var BestDiscountName = ''
        }
        if (selectButtonHidden[8] != null) {
            var pc_id = selectButtonHidden[8]
            var additionalClass = 'selectDiscount'
            var updatePriceInputId = 'radio' + idPostfix
            // var updatePriceInputId = 'radio' + idPostfix + '_' + pc_id
            var selectRoomId = 'buttonAvailable' + i + valueRoomTypeId + '_' + pc_id
        } else {
            var pc_id = ''
            var additionalClass = ''
            var updatePriceInputId = 'radio' + idPostfix
            var selectRoomId = 'buttonAvailable' + i + valueRoomTypeId
        }
    }
    ...
    return (
        <>
            <input type="hidden"
                ...
                Value="0" id={updatePriceInputId} />
            <input type="button"
                onClick={() => roomSelection(`${i}`, `${idPostfix}`, `${valueRoomTypeId}`, `${nextId}`, `${isDealApplied}`, 'this')}
                Value={'Select Room'} title="Select Room" id={selectRoomId} className={`selectRoomTypeId${i}${valueRoomTypeId} selectRoom${i} btn btn-success but_bg1 F16 ${additionalClass} notranslate roomSelectButton`} />
        </>
    )
}
