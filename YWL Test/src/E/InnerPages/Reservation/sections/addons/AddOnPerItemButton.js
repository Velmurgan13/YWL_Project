...
const AddOnPerItemButton = ({ val2, updateGrandTotal, addonids, handleperNightAddon, addOnModalPopup, setAddonPerItemButtonFlag }) => {
  function selectAddon(id, addName, tax, addonPrice) {
    ...
    if (document.getElementById("perItemAddon_" + id).checked) {
      ...
      if (document.getElementById("Add_Addon_" + id)) {
        ...
        // document.getElementById("Add_Addon_" + id).classList.add("btn-danger");
        ...
        if (document.getElementById("perNightAddon_" + id)) {
          document.getElementById("perNightAddon_" + id).removeAttribute("enabled", "");
          document.getElementById("perNightAddon_" + id).setAttribute("disabled", "");
        }
  ...
  return (
    <>
      <input
        type="button"
        value="Remove"
        id={"removeAddonPerItem_" + val2.Id}
        style={{ display: "none" }}
        className="btn btn-danger removeAddon"
        onClick={() =>
          selectAddon(
            `${val2.Id}`,
            `${val2.Name}`,
...
export default AddOnPerItemButton;
